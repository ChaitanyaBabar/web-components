...
export const AnnotateUtilityContext = createContext<AnnotationUtility>('annotation-utility-context');

export interface AnnotationUtility {
  startAnnotationParsing(element: any, annotationList: any): void;
  removeAnnotations(element: any): void;
}
 ...
 */
export class AnnotationUtilityImpl implements AnnotationUtility {

    private _statesToAnnotate = ['BX_INSTANCE_TASKS_COMPLETED'];
     ...
     */
    private processElementForAnnotation(element: any, annotationList: any): any {
        let processedResult = {
            toTraverseDOM: true,
        ...
        if (elementId && annotationList && annotationList.length > 0) {
            annotationList.some((annotation: any) => {
     ...
     */
    private createAnnotationWrapper(currentProcessingElement: any, annotatedElement: any): void {
        ...
        rect.addEventListener('click', (event: any) => {
     ...
     * @param annotatedElement - The annotation data
     */
    private createAnnotationIcon(wrapper: any, currentProcessingElement: any, annotatedElement: any): void {
        // Create the audit annotation icon web component
        const iconComponent = document.createElement('bpme-audit-annotation-icon') as any;
    ...
    removeAnnotations(element: any): void {
        ...
        annotatedElements.forEach((annotatedElement: any) => {
